...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('-d', '--sys_device_ids', type=eval, default=(0,1,))  # 更改(0,)为(0,1,)，使用两个显卡训练
        parser.add_argument('--set_seed', type=str2bool, default=False)
        ## dataset parameter
        parser.add_argument('--dataset', type=str, default='rap',
                choices=['peta','rap', 'pa100k', 'rap2'])  # 更改默认数据集为rap
        parser.add_argument('--split', type=str, default='trainval',
                            choices=['trainval', 'train'])
        parser.add_argument('--test_split', type=str, default='test')
        parser.add_argument('--partition_idx', type=int, default=0)  # 使用第几个数据集划分
        parser.add_argument('--resize', type=eval, default=(224, 224))  # ImageNet数据集标准预处理大小
        parser.add_argument('--mirror', type=str2bool, default=True)
        parser.add_argument('--batch_size', type=int, default=32)  # 两块显卡，设置batch_size为256
        parser.add_argument('--workers', type=int, default=4)
        # model
        parser.add_argument('--num_att', type=int, default=35)  #  属性个数，即最后一层分类数
        ...
        parser.add_argument('--sgd_weight_decay', type=float, default=0.0005)  # 衰减5e-4
        ...
        if not datasets.has_key(args.dataset) or not partitions.has_key(args.dataset):  # 数据集标签索引或者数据集划分索引有一个不存在
        ...
        # for model
        model_kwargs = dict()
        model_kwargs['num_att'] = args.num_att  # 用于分类的属性数，实际上就是最后一层的分类数
...
train_set = AttDataset(
    dataset = cfg.dataset,  # 数据集标签.pkl文件的路径 
...
for epoch in range(start_epoch, cfg.total_epochs):
    ...
    if (epoch + 1) % cfg.epochs_per_save == 0 or epoch+1 == cfg.total_epochs:
        ckpt_file = os.path.join(cfg.exp_dir, 'model', 'ckpt_epoch%d.pth'%(epoch+1))  # 保存模型
